...
function s.cfilter(c)
	return c:IsSetCard(0x7c9) and c:IsMonster() and aux.SpElimFilter(c,true)
end
...
function s.atkop(e,tp,eg,ep,ev,re,r,rp)
	...
	local ct=Duel.GetMatchingGroupCount(aux.FaceupFilter(Card.IsRace,RACE_DRAGON),tp,LOCATION_REMOVED,0,nil)
	if #g>0 then
		g:ForEach(s.op,e:GetHandler(),-300*ct)
	end
end
...
function s.spfilter(c,e,tp)
	return c:IsFaceup() and c:IsLevelBelow(12) and c:IsSetCard(0x7c9) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
